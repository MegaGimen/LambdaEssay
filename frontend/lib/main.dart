...
class _GraphPageState extends State<GraphPage> with TickerProviderStateMixin {
  ...
  double _uiScale = 1.0;
  double _rowHeight = 160.0;
  double _laneWidth = 120.0;
  ...
  Future<void> _connectWebSocket() async {
    ...
    try {
      ...
      setState(() {
        _comConnected =
            true; // Assume connected if websocket is open, server will send updates
      });
                      ...
                      itemBuilder: (context, index) {
                        if (index < repos.length) {
                          ...
                          if (repoList.isNotEmpty) {
                            final info = repoList.firstWhere(
                                (e) => e['name'] == repo,
                                orElse: () => {});
              ...
              ElevatedButton(
                onPressed: () {
                  if (isCreatingNew) {
                    ...
                    if (repoList.isNotEmpty) {
                      final info = repoList.firstWhere(
                          (e) => e['name'] == selectedRepo,
                          orElse: () => {});
                      isFolder = info['isFolder'] == true;
                    }
                    Navigator.pop(
                        ctx, {'name': selectedRepo, 'isFolder': isFolder});
                  }
                },
                child: const Text('确定'),
              ),
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            labelText:
                                isFolderProject ? '文件夹路径' : 'docx文件路径或解包文件夹路径',
                          ),
                        ),
                      ),
                      IconButton(
                        icon: Icon(isFolderProject
                            ? Icons.folder
  ...
  Future<void> _doImportProject(
    ...
    try {
      ...
      for (final repo in subRepos) {
        ...
        if (dPath != null &&
            (p.equals(dPath, targetFilePath) || dPath == targetFilePath)) {
          targetRepo = repo;
          break;
        }
      }
      ...
      if (targetRepo != null) {
        ...
        final hash =
            md5.convert(utf8.encode(targetFilePath)).toString().substring(0, 8);
        targetRepoPath =
            p.join(gitdocxDir, currentProjectName!, '${name}_$hash');
      }
                ...
                if (data != null) ...[
                  ...
                  const Text('视图设置',
                      style: TextStyle(
                          fontWeight: FontWeight.bold, color: Colors.grey)),
                  const SizedBox(height: 8),
                  _buildActionButton(Icons.home, '返回主视角', _resetView),
                  _buildActionButton(Icons.history, '历史备份', () {
                    if (currentProjectName == null || pathCtrl.text.isEmpty) {
                      return;
                    }
                    Navigator.push(
                        context,
                  ...
                  ...() {
                    ...
                    return branches.asMap().entries.map((e) {
                      final i = e.key;
                      final b = e.value;
                      ...
                      return Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 8),
                            Expanded(
                                child: Text(Branch.decodeName(b.name),
                                    overflow: TextOverflow.ellipsis)),
